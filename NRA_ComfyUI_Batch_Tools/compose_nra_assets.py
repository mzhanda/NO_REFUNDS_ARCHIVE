...
def load_image(path: str, size: Optional[tuple] = (CANVAS_SIZE, CANVAS_SIZE)) -> Optional[Image.Image]:
    """Load an image, convert to RGBA, resize to target size. Returns None on failure.
    Pass size=None to skip resizing (e.g. for cropped props)."""
...
def resolve_asset(asset_id: str, category: str) -> Optional[str]:
    ...
    # For props, prefer cropped version if available
    if category == "prop":
        cropped_path = os.path.join(ASSETS_DIR, "props_cropped", filename)
        if os.path.exists(cropped_path):
            return cropped_path

    # For legendary_accent stickers (prop_*), prefer cropped version
    if category == "legendary_accent" and filename.startswith("prop_"):
        cropped_path = os.path.join(ASSETS_DIR, "legendary_accent_cropped", filename)
        if os.path.exists(cropped_path):
            return cropped_path
...
def compose_image(row: dict) -> Optional[Image.Image]:
    ...
    # Track prop index for offset positioning
    prop_index = 0
    ...
    for csv_field, category, label in layers:
        ...
        for aid in asset_ids:
            ...
            # Determine if this asset should be loaded at original size (cropped)
            # - regular props: always cropped
            # - legendary_accent stickers (prop_*): cropped, placed like props
            # - legendary_accent overlays (overlay_*): full canvas
            is_cropped_sticker = (
                category == "prop" or
                (category == "legendary_accent" and "legendary_accent_cropped" in asset_path)
            )

            if is_cropped_sticker:
                layer_img = load_image(asset_path, size=None)
            else:
                layer_img = load_image(asset_path)
            if layer_img is None:
                continue

            # ── Cropped stickers (props + legendary stickers): place randomly along top edge ──
            if is_cropped_sticker:
                pw, ph = layer_img.size

                # Regular props: scale up slightly for better visibility
                if category == "prop":
                    scale = 1.25
                    new_w = int(pw * scale)
                    new_h = int(ph * scale)
                ...
                # Y: regular props lower, legendary stickers stay at top edge
                if category == "prop":
                    y_pos = random.randint(20, 60)
                else:
                    y_pos = random.randint(-10, 30)

                # X: spread across full width, keeping sticker on canvas
                x_pos = random.randint(10, max(10, CANVAS_SIZE - pw - 10))

                canvas.paste(layer_img, (x_pos, y_pos), layer_img)
                print(f"  ✓ {label}: {aid} (cropped {pw}x{ph}, top edge x={x_pos} y={y_pos})")
                if category == "prop":
                    prop_index += 1
